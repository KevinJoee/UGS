...
def render_set(model_path, load2gpt_on_the_fly, name, iteration, views, gaussians, pipeline, background, deform):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        d_xyz, d_rotation, d_scaling = deform.step(xyz.detach(), time_input, feature=gaussians.feature, is_training=False, motion_mask=gaussians.motion_mask)
...
def interpolate_all(model_path, load2gpt_on_the_fly, name, iteration, views, gaussians, pipeline, background, deform):
    ...
    for i, pose in enumerate(tqdm(render_poses, desc="Rendering progress")):
        ...
        d_xyz, d_rotation, d_scaling = deform.step(xyz.detach(), time_input, feature=gaussians.feature, motion_mask=gaussians.motion_mask)
        results = render(view, gaussians, pipeline, background, d_xyz, d_rotation, d_scaling)
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, skip_train: bool, skip_test: bool, mode: str):
    with torch.no_grad():
        ...
        deform = DeformModel(deform_type=dataset.deform_type, is_blender=dataset.is_blender, skinning=dataset.skinning, hyper_dim=dataset.hyper_dim, node_num=dataset.node_num)
        ...
        gaussians = GaussianModel(dataset.sh_degree, fea_dim=gs_fea_dim)
        ...
        if not skip_train:
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, deform)

        if not skip_test:
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, deform)
